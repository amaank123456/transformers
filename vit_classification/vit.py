...
class ViT(nn.Module):
    ...
    def __init__(self, patch_dim, d_model, d_ff, num_heads, num_layers, num_patches, num_classes, device = 'cuda'):
        ...
        self.positional_encoding = PositionalEncoding(self.d_model, max_len=self.num_patches+1)  # TODO (use the positional encoding from the transformer captioning solution)
    ...
    def patchify(self, images):
        ...
        patches = images.unfold(2,self.patch_dim,self.patch_dim).unfold(3,self.patch_dim,self.patch_dim).contiguous().view(N, C, self.num_patches, self.patch_dim, self.patch_dim).permute(0,2,1,3,4).contiguous().view(N, self.num_patches, -1)
    ...
    def forward(self, images):
        ...
        output = torch.cat([self.cls_token.expand(images.shape[0], 1, self.d_model), patches_embedded], dim=1) # TODO (append a CLS token to the beginning of the sequence of patch embeddings)
